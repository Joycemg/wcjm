...
<article
    class="card-game mesa-card"
    id="mesa-card-{{ $mesa->id }}"
    data-mesa-id="{{ $mesa->id }}"
    data-opens-ts="{{ $opensTs }}"
...
@media (prefers-reduced-motion: reduce){ .mesa-card .bar>span{transition:none} }
</style>
@endpush
...
(function(){
  ...
  ready(function(){
    // Meta opcional para corregir desfase cliente/servidor:
    // <meta name="server-now-ms" content="{{ (int) now('UTC')->getTimestampMs() }}">
    ...
    function initCard(root){
      initEnableAt(root);
      initCountdown(root);
      // Anti doble-submit básico
      root.querySelectorAll('form').forEach(f=>{
        f.addEventListener('submit', ()=>{
          const b = f.querySelector('button');
          if(b){ b.disabled = true; b.classList.add('is-disabled'); }
        }, {once:true});
      });
    }

    // Boot simple (sin polling)
